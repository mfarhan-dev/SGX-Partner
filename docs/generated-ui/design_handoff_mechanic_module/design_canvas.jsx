...
// ─────────────────────────────────────────────────────────────
// Per-artboard error isolation. Without it, an error thrown while
// ONE artboard's content renders (a null context read, a missing
// provider, a bad component) unmounts the whole React root and
// blanks every artboard on the canvas. This boundary confines the
// blast radius to the offending artboard, which shows an inline
// placeholder while its siblings keep rendering. Happy path is a
// pure passthrough (no wrapper DOM, no layout change). Keyed by
// artboard id at each call site so focus-mode navigation between
// artboards mounts a fresh boundary rather than sticking on a
// prior error.
// ─────────────────────────────────────────────────────────────
class DCArtboardBoundary extends React.Component {
  constructor(props) {
    super(props);
  ...
  componentDidCatch(error, info) {
    console.error('[design-canvas] artboard failed to render:', error, info);
  }
  ...
  render() {
    ...
    return (
      <div style={{ height: '100%', boxSizing: 'border-box', padding: 20, display: 'flex',
        flexDirection: 'column', gap: 6, alignItems: 'center', justifyContent: 'center',
        textAlign: 'center', background: '#fdf3f2', color: '#a3352b', fontFamily: DC.font }}>
        <div style={{ fontSize: 13, fontWeight: 600 }}>This screen failed to render</div>
        <div style={{ fontSize: 11, opacity: 0.8, wordBreak: 'break-word', maxWidth: '100%' }}>{msg}</div>
      </div>
    );
  }
}
  ...
  return (
    <div ref={ref} data-dc-slot={id} style={{ position: 'relative', flexShrink: 0 }}>
      ...
      <div ref={cardRef} className="dc-card"
        ...
        <DCArtboardBoundary key={`${sectionId}/${id}`}>
      ...
      <div
        ...
        <div onClick={(e) => e.stopPropagation()} style={{ width: width * scale, height: height * scale, position: 'relative' }}>
          <div style={{ width, height, transform: `scale(${scale})`, transformOrigin: 'top left', background: '#fff', borderRadius: 2, overflow: 'hidden',
            ...
            <DCArtboardBoundary key={`${sectionId}/${aid}`}>
