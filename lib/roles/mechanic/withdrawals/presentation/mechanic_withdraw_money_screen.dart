...
class MechanicWithdrawMoneyScreen extends ConsumerStatefulWidget {
  const MechanicWithdrawMoneyScreen({super.key});

  @override
  ConsumerState<MechanicWithdrawMoneyScreen> createState() =>
      _MechanicWithdrawMoneyScreenState();
}

class _MechanicWithdrawMoneyScreenState
    extends ConsumerState<MechanicWithdrawMoneyScreen> {
  ...
  Future<void> _submit(
    ...
    try {
      ...
      if (!mounted) return;
      context.go('/mechanic/withdrawals/${withdrawal.id}');
    } catch (error) {
      if (!sheetContext.mounted) return;
      Navigator.pop(sheetContext);
      setState(() {
