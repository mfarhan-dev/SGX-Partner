...
/// already there.
///
/// The shell's own bottom nav bar/FAB (MechanicShell) occupies this
...
Future<void> showMechanicWithdrawMoneySheet(
  ...
  try {
    await showModalBottomSheet<void>(
      ...
      showDragHandle: true,
      builder: (context) => const _MechanicWithdrawMoneySheet(),
    );
  } finally {
...
class _MechanicWithdrawMoneySheet extends ConsumerStatefulWidget {
  const _MechanicWithdrawMoneySheet();

  @override
  ConsumerState<_MechanicWithdrawMoneySheet> createState() =>
      _MechanicWithdrawMoneySheetState();
}

class _MechanicWithdrawMoneySheetState
    extends ConsumerState<_MechanicWithdrawMoneySheet> {
  ...
  void _openPayoutSettings(BuildContext context) {
    ...
    router.push('/mechanic/payout-method');
  }
  ...
  Future<void> _submit(
    ...
    try {
      ...
      context.push('/mechanic/withdrawals/${withdrawal.id}');
    } catch (error) {
      if (!dialogContext.mounted) return;
      Navigator.pop(dialogContext);
      setState(() {
